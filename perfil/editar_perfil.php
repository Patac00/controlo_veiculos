...
// Buscar dados do utilizador e empresa
$sql = "SELECT nome, email, empresa_id, cargo FROM utilizadores WHERE id_utilizador = $id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $cargo = isset($_POST['cargo']) && $_POST['cargo'] !== '' ? mysqli_real_escape_string($con, $_POST['cargo']) : "NULL";
    ...
    if (isset($_FILES['foto_perfil']) && $_FILES['foto_perfil']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedfileExtensions)) {
            ...
            $empresa_clean = $empresa_id !== "NULL" ? preg_replace('/\s+/', '', strtolower($empresa_id)) : '';
    ...
    if (empty($erro)) {
        ...
        if ($foto_sql !== null) $campos[] = $foto_sql;
        ...
        if (empty($erro)) {
            ...
            if (mysqli_query($con, $sql_update)) {
                ...
                $user['nome'] = $nome;
                $user['email'] = $email;
                $user['empresa_id'] = $empresa_id !== "NULL" ? $empresa_id : null;
                $user['cargo'] = $cargo !== "NULL" ? $cargo : '';
                if ($foto_sql !== null) $user['foto_perfil'] = $newFileName;
            } else {
    ...
    <form method="post" action="" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="empresa_id" class="form-select">
                ...
                <?php mysqli_data_seek($result_empresas, 0); while($empresa = mysqli_fetch_assoc($result_empresas)): ?>
                    <option value="<?= $empresa['empresa_id'] ?>" <?= ($empresa['empresa_id'] == $user['empresa_id']) ? 'selected' : '' ?>>
                        <?= htmlspecialchars($empresa['nome']) ?>
                        <?php if (!empty($empresa['unidades'])): ?> - Unidade <?= htmlspecialchars($empresa['unidades']) ?><?php endif; ?>
                    </option>
                <?php endwhile; ?>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="cargo" class="form-select">
                <option value="">-- Em branco --</option>
                <?php
                $cargos = ['admin','informática','contabilidade'];
                foreach($cargos as $c) {
